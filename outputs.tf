...
output "inventory_instances" {
  ...
  value = {for i in (var.var.instance_number): aws_instance.server[i].tags.Name => "${aws_instance.server[i].public_ip} ansible_ssh_private_key_file=${var.private_key_path}-${aws_instance.server[i].id}.pem ansible_user=${var.ansible_user}"}
}
/*
output "ssh-connect" {
